<template>
  <div>
    ...
    <div class="flex justify-between p-4 bg-white dark:bg-gray-800 dark:shadow-soft-dark-xl shadow rounded-xl">
      <h4 class="text-lg font-semibold dark:text-white">Partner Types</h4>
    ...
    <div :class="['fixed top-0 right-0 w-96 h-screen z-[990] bg-white dark:bg-gray-800 shadow-lg transform transition-transform overflow-y-auto p-6',
      ...
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-bold dark:text-white">{{ editingId ? 'Edit Partner Type' : 'Add Partner Type' }}</h3>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[999]">
      <div class="bg-white dark:bg-gray-800 p-6 rounded shadow max-w-md w-full">
        <h3 class="text-lg font-semibold mb-3 dark:text-white">Delete Partner Type</h3>
...
const onDragEnd = async () => {
  try {
    ...
    await axios.post('/api/partner-types/reorder', { order: ordered })
...
const saveType = async () => {
  try {
    if (editingId.value) {
      await axios.put(`/api/partner-types/${editingId.value}`, form.value)
      toast.success('Partner type updated')
    } else {
      await axios.post('/api/partner-types', form.value)
    ...
    if (error.response?.status === 422) {
      ...
      toast.error('Error saving partner type')
    }
  }
}
...
const deleteType = async () => {
  try {
    await axios.delete(`/api/partner-types/${typeToDelete.value.id}`)
    toast.success('Partner type deleted')
    await fetchTypes()
  } catch {
    toast.error('Error deleting partner type')
  } finally {
